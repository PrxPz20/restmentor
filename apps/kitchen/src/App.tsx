...
const ORDERS_STORAGE_KEY = 'rm_kitchen_orders';
const MAX_ORDERS = 30;
const NEW_HIGHLIGHT_MS = 6000;
const MOD_HIGHLIGHT_MS = 60000; // 60s before removed/updated items normalize
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', background: 'var(--color-bg)' }}>
      <div style={{ background: 'var(--color-white)', borderRadius: 'var(--radius-card)', boxShadow: 'var(--shadow-card)', padding: '40px 32px', width: '100%', maxWidth: '400px', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: '20px', fontWeight: 600, marginBottom: '8px', fontFamily: 'var(--font)' }}>Kitchen Display Setup</h1>
            ...
            <div key={idx} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '6px' }}>
                ...
                <span style={{
                  ...
                  color: isRemoved
                    ? 'var(--color-removed-text)'
                    : isUpdated
                      ? 'var(--color-updated-text)'
                      : 'var(--color-primary)',
                }}>
                  {item.name}
                </span>
              ...
              {/* Quantity change hint */}
              {isUpdated && item.previousQuantity !== undefined && (
                <p style={{ fontSize: '12px', fontWeight: 500, fontFamily: 'var(--font)', color: 'var(--color-updated-text)', marginTop: '3px', marginLeft: isUpdated ? '20px' : '0' }}>
...
export default function KitchenDisplay() {
  ...
  // Orders stored as a Map keyed by tableLabel::roundNumber
  // This lets modifications find and update the exact right card
  ...
  useEffect(() => {
    ...
    socket.on('order:new', (data: IncomingOrder) => {
      ...
      // Remove "new" highlight after timeout
      setTimeout(() => {
        setOrderMap(prev => {
          const next = new Map(prev);
    ...
    // ── Modification: update existing card in place ──
    socket.on('order:modified', (data: OrderModification) => {
      ...
      setOrderMap(prev => {
        ...
        if (!card) return prev; // Card not found — ignore (wrong destination or refreshed)
      ...
      // After MOD_HIGHLIGHT_MS: normalize updated items, remove deleted ones, clear modified flag
  ...
  const orders = Array.from(orderMap.values()).reverse(); // Newest first
